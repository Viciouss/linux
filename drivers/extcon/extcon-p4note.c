...
/**
 * Based on the px-switch.c of the original vendor kernel.
 *
 ...
 * connection.
 *
 * USB path	SEL_0	SEL_1	SEL_CP (SAFEOUT)
 * ADC Check	1	0	no change
 * TA		0	0	no change
 * CP		0	1	set to 1
 * AP		1	1	set to 0
 * Default	1	1	no change
 *
 * ADC Check: used to determine cable type for charger, short activation only
 * TA: set if there is no USB connection,
 * CP: initial value if GPIOs are set up like this (from bootloader probably)
 * AP: set for OTG power, remains set
 * Default: probably unused
 *
...
#define log_connceted_state(data) dev_dbg(data->dev, "accessory(%d) + dock(%d) + charger(%d)\n", \
		...
		data->dock.last_state, \
		data->charger.last_state)
...
void accessory_power(struct p4note_extcon_data *data, u8 token, bool active)
{
	...
	if (active) {
		if (data->acc_en_token) {
			dev_dbg(data->dev, "Board: Keyboard dock is connected.\n");
		...
		if (token == 0) {
			...
			data->acc_en_token &= ~(1 << token);
			if (data->acc_en_token == 0)
				gpiod_set_value(data->accessory_enable, 0);
		}
	}

	dev_dbg(data->dev, "Board : (%d,%d) %s\n",
...
static void pmic_safeout2(struct p4note_extcon_data *data, int onoff)
{
	...
	if (onoff) {
		if (!gpiod_get_value(cp)) {
			...
			dev_dbg(data->dev, "changed usb cp to ON\n");
		}
	} else {
		if (gpiod_get_value(cp)) {
			gpiod_set_value(cp, onoff);
			dev_dbg(data->dev, "changed usb cp to OFF\n");
		}
	}
}
...
static void usb_apply_path(struct p4note_extcon_data *data)
{
	...
	dev_dbg(data->dev, "current gpio before changing : sel0:%d sel1:%d sel_cp:%d\n",
	...
	if (path & USB_PATH_ADCCHECK) {
		dev_dbg(data->dev, "ADC path hit, setting gpio to 1/0\n");
	...
	if (path & USB_PATH_TA) {
		dev_dbg(data->dev, "TA path hit, setting gpio to 0/0\n");
	...
	if (path & USB_PATH_CP) {
		dev_dbg(data->dev, "CP path hit, setting gpio to 0/1\n");
		gpiod_set_value(sel0, 0);
	...
	if (path & USB_PATH_AP) {
		dev_dbg(data->dev, "AP path hit, setting gpio to 1/1\n");
		gpiod_set_value(sel0, 1);
		gpiod_set_value(sel1, 1);
		goto out_ap;
	}
	...
	/* default */
	dev_dbg(data->dev, "no path was hit, setting gpio to default 1/1\n");
	gpiod_set_value(sel0, 1);
	gpiod_set_value(sel1, 1);
...
out_nochange:
	dev_dbg(data->dev, "%x safeout2 no change\n", path);
	return;
...
static void p4note_set_cable_state(struct device *dev, struct extcon_dev *edev,
				   struct p4note_adc_cond *cond, bool status)
{
	int err = 0;
	...
	if (cond->type[1] > EXTCON_NONE) {
		...
		if (err)
			dev_err(dev, "could not change type0 to %d: %d", cond->type[1], err);
	}
}
...
static void update_accessory(struct p4note_extcon_data *data)
{
	if (gpiod_get_value(data->accessory.desc)) {
		if (data->accessory.last_state == GPIO_OFF) {
			dev_dbg(data->dev, "connecting accessory...\n");
			data->accessory.last_state = GPIO_ON;
			start_acc_worker(data, 0);
		}
	} else {
		if (data->accessory.last_state == GPIO_ON) {
			dev_dbg(data->dev, "disconnecting accessory...\n");
			data->accessory.last_state = GPIO_OFF;
			p4note_set_cable_state(data->dev, data->edev,
					       data->current_accessory, false);
...
static void p4note_update_charger_path(struct p4note_extcon_data *data, bool charger_state)
{
	if (data->current_charger->type[0] == EXTCON_CHG_USB_DCP
		...
		dev_dbg(data->dev, "charger attached and path is not AP -> set path to TA");
		mutex_lock(&data->usb_mutex);
		usb_switch_set_path(data, USB_PATH_TA);
		mutex_unlock(&data->usb_mutex);
	} else if ((data->current_path & USB_PATH_TA) != 0) {
		dev_dbg(data->dev, "charger changed and path is TA -> clear TA flag");
		mutex_lock(&data->usb_mutex);
...
static void read_accessory_adc_worker(struct work_struct *work)
{
	...
	if (err) {
		dev_info(data->dev, "error reading the adc value for the accessory: %d\n", err);
		return;
	}

	dev_dbg(data->dev, "read accessory adc value = %d\n", adc_val);
	...
	if (data->current_accessory != &acc_adc_conditions[condition_counter]) {
		...
		dev_dbg(data->dev, "accessory changed, setting up new type");
		...
		if (acc_adc_conditions[condition_counter].type[0]) {
			...
			dev_dbg(data->dev, "device attached with type1=%d and type2=%d\n",
				data->current_accessory->type[0],
				data->current_accessory->type[1]);

			if (data->current_accessory->type[0] == EXTCON_USB_HOST
			    || data->current_accessory->type[1] == EXTCON_USB_HOST)
				enable_usb(data);
			else
				disable_usb(data);

			p4note_set_cable_state(data->dev, data->edev,
					       data->current_accessory, true);
		} else {
			dev_err(data->dev, "unknown accessory attached, adc value %d\n", adc_val);
			disable_usb(data);
		}
	}

	if (data->accessory.last_state == GPIO_ON && data->dock.last_state == GPIO_ON) {
		// both the dock and the accessory are connected, poll the adc value to get updates
		dev_dbg(data->dev, "rescheduling adc task as accessory and dock line are set\n");
...
static void read_charger_adc_worker(struct work_struct *work)
{
	...
	if (data->current_charger != &charger_adc_conditions[condition_counter]) {
		...
		dev_dbg(data->dev, "power changed, setting up new type");
		...
		if (charger_adc_conditions[condition_counter].type[0]) {
			...
			dev_dbg(data->dev, "the connected charger is of type[0] %d type[1] %d\n",
			...
			dev_dbg(data->dev, "unknown charger with adc value %d\n", adc_val);
		}
	}
}
...
static int p4note_extcon_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->headphone_iio_chan)) {
		...
		dev_err(data->dev, "headphone error %ld\n", PTR_ERR(data->headphone_iio_chan));
		return PTR_ERR(data->headphone_iio_chan);
	}
	dev_dbg(data->dev, "headphone adc found\n");
	...
	if (IS_ERR(data->charger_iio_chan)) {
		...
		dev_err(data->dev, "charger error %ld\n", PTR_ERR(data->charger_iio_chan));
		return PTR_ERR(data->charger_iio_chan);
	}
	dev_dbg(data->dev, "charger adc found\n");
	...
	if (IS_ERR(data->acc_iio_chan)) {
		...
		dev_err(data->dev, "accessory error %ld\n", PTR_ERR(data->acc_iio_chan));
		return PTR_ERR(data->acc_iio_chan);
	}
	dev_dbg(data->dev, "accessory adc found\n");
	...
	dev_dbg(data->dev, "delayed work initialized\n");
