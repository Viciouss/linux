...
int calc_checksum(struct wacom_i2c *wac_i2c, unsigned char *flash_data)
{
	unsigned long i;
	...
	for (i = 0; i < 4; i++) {
		if (checksum_data[i] != wac_i2c->firmware_checksum[i+1])
			return -ERR_HEX;
	}
...
int wacom_i2c_flash_cmd(struct wacom_i2c *wac_i2c)
{
	...
	if (ret >= 0) {
		...
		if (ret < 0) {
			dev_err(&wac_i2c->client->dev, "%s: Sending flash command failed\n", __func__);
			return -1;
		}
		dev_dbg(&wac_i2c->client->dev, "%s: flash send?:%d\n", __func__, ret);
		msleep(270);
	}
...
int wacom_i2c_flash_query(struct wacom_i2c *wac_i2c, u8 query, u8 recvdQuery)
{
	...
	ret = wacom_i2c_send(wac_i2c, &flashq, len, true);
	if (ret < 0) {
		dev_err(&wac_i2c->client->dev, "%s: query unsent:%d\n", __func__, ret);
		return -1;
	}
	...
	do {
		...
		if (i > RETRY)
			return -1;
		dev_dbg(&wac_i2c->client->dev, "%s: ret:%d flashq:%x\n", __func__, ret, flashq);
	} while (recvdQuery == 0xff && flashq != recvdQuery);
	dev_dbg(&wac_i2c->client->dev, "%s: query:%x\n", __func__, flashq);
...
int wacom_i2c_flash_end(struct wacom_i2c *wac_i2c)
{
	...
	dev_dbg(&wac_i2c->client->dev, "%s: Digitizer activated\n", __func__);
			  ...
			  u8 cmd_block, u8 endBlock)
{
	...
	for (i = cmd_block; i >= endBlock; i--) {
		...
		if (ret < 0) {
			dev_err(&wac_i2c->client->dev, "%s: Erase failed\n", __func__);
			return -1;
		}
		...
		do {
			...
			if (j > RETRY || flashq == 0x84 || flashq == 0x88
				...
				/*
				   ...
				   0x90:Checksum error
				 */
				dev_err(&wac_i2c->client->dev, "%s: Error:%x\n", __func__, flashq);
				return -1;
			}
		...
		if (printk_timed_ratelimit(&swtich_slot_time, 5000))
			dev_dbg(&wac_i2c->client->dev, "%s: Erasing at %d, ", __func__, i);
		/*sleep */
		msleep(1);
	}
	dev_dbg(&wac_i2c->client->dev, "%s: Erasing done\n", __func__);
	return ret;
}
			  ...
			  u8 size, unsigned long maxAddr)
{
	...
	for (ulAddr = startAddr; ulAddr <= maxAddr; ulAddr += BLOCK_SIZE_W) {
		...
		*/
		for (k = 0; k < RETRY_TRANSFER; k++) {
			...
			if (ret < 0 || k == (RETRY_TRANSFER - 1)) {
				dev_err(&wac_i2c->client->dev, "%s: Write process aborted\n", __func__);
		...
		do {
			...
			if (j > RETRY || buf[0] == 0x90) {
				...
				dev_err(&wac_i2c->client->dev, "%s: Error: %x , 0x%lx(%d)\n",
		...
		*/
		...
		if (printk_timed_ratelimit(&swtich_slot_time, 5000))
			dev_dbg(&wac_i2c->client->dev, "%s: Written on:0x%lx", __func__, ulAddr);
		msleep(1);
	}
			   ...
			   u8 size, unsigned long maxAddr)
{
	...
	for (ulAddr = startAddr; ulAddr <= maxAddr; ulAddr += BLOCK_SIZE_W) {
		...
		*/
		...
		if (printk_timed_ratelimit(&swtich_slot_time, 5000))
			dev_dbg(&wac_i2c->client->dev, "%s: Verified:0x%lx\n", __func__, ulAddr);
		msleep(1);
	}

	if (calc_checksum(wac_i2c, Binary)) {
		dev_dbg(&wac_i2c->client->dev, "%s: check sum not matched\n", __func__);
		return -ERR_HEX;
	}
...
int wacom_i2c_flash(struct wacom_i2c *wac_i2c)
{
	...
	if (Binary == NULL) {
		dev_err(&wac_i2c->client->dev, "%s: Data is NULL. Exit.\n", __func__);
		return -1;
	}
	...
	dev_dbg(&wac_i2c->client->dev, "%s: flashEnter:%d\n", __func__, ret);
	msleep(10);

	blver = wacom_i2c_flash_BLVer(wac_i2c);
	dev_dbg(&wac_i2c->client->dev, "%s: blver:%d\n", __func__, blver);
	msleep(10);

	mcu = wacom_i2c_flash_mcuId(wac_i2c);
	dev_dbg(&wac_i2c->client->dev, "%s: mcu:%x\n", __func__, mcu);
	if (wac_i2c->ic_mpu_ver != mcu) {
		dev_dbg(&wac_i2c->client->dev, "%s: mcu:%x\n", __func__, mcu);
		ret = -ENXIO;
		goto mcu_type_error;
	}
	...
	switch (mcu) {
	case MPUVER_W8501:
		dev_dbg(&wac_i2c->client->dev, "%s: flashing for w8501 started\n", __func__);
	...
	case MPUVER_514:
		dev_dbg(&wac_i2c->client->dev, "%s: Flashing for 514 started\n", __func__);
	...
	case MPUVER_505:
		dev_dbg(&wac_i2c->client->dev, "%s: Flashing for 505 started\n", __func__);
	...
	default:
		dev_dbg(&wac_i2c->client->dev, "%s: default called\n", __func__);
		break;
	}
	...
	if (fw_update_enable) {
		...
		do {
			...
			ret = wacom_i2c_flash_erase(wac_i2c, FLASH_ERASE,
				    cmd_addr, END_BLOCK);
			if (ret < 0) {
				dev_err(&wac_i2c->client->dev, "%s: error - erase\n", __func__);
				continue;
			}
			msleep(20);
			...
			if (ret < 0) {
				dev_err(&wac_i2c->client->dev, "%s: error - writing\n", __func__);
				continue;
			}
			msleep(20);
			...
			if (ret == -ERR_HEX)
				dev_dbg(&wac_i2c->client->dev, "%s: firmware is not valied\n", __func__);
			else if (ret < 0) {
				dev_err(&wac_i2c->client->dev, "%s: error - verifying\n", __func__);
				continue;
			} else
				valid_hex = true;
		...
		if (cnt < 10)
			dev_dbg(&wac_i2c->client->dev, "%s: Firmware successfully updated\n", __func__);
	}
	msleep(1);
...
mcu_type_error:
	...
	if (ret < 0)
		dev_err(&wac_i2c->client->dev, "%s: error - wacom_i2c_flash_end\n", __func__);
